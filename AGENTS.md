...
- ⏳ ยังไม่ sync prod — ใช้ `sync-to-prod.ps1` + `redeploy.bat` ตาม checklist

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: ปุ่มขั้นตอนใหญ่แสดงจำนวนรายการ + คลิกขวาเลือกย้อน/ทำซ้ำเฉพาะบางรายการในกลุ่ม ✅**)
- **ปุ่มขั้นตอนใหญ่แสดงจำนวน** — tooltip "ย้อนขั้นตอนใหญ่ — ย้อน N รายการพร้อมกัน · คลิกขวาเพื่อเลือก" (และ redo) อัปเดตสด + **badge ตัวเลขบนปุ่ม** เมื่อกลุ่ม > 1 — refactor `groupOf()` + `applyUndoEntries`/`applyRedoEntries` ใช้ร่วม (ยกเลิกการกลับลำดับใน helper — caller ส่งลำดับ apply ชัดเจน)
- **คลิกขวา = เมนูเลือกรายการ** — checkbox รายการในกลุ่ม (ใหม่สุดบนสุด) + label/เวลา/detail · สรุป "ย้อน N จาก M รายการบนสุด" สดจากที่ติ๊ก · ยืนยัน = ย้อน/ทำซ้ำเฉพาะที่เลือก (เลือกต่อจากบนสุดเท่านั้น — ก่อน/หลังเป็น snapshot เต็ม) · เปิดอันหนึ่งปิดอีกอัน + คลิกนอกปิด
- i18n 3 ภาษา 8 คีย์ — typecheck 0 + build ผ่าน + integration 18/18 — เทสจริง: ปิด 3 กฎ → tooltip "ย้อน 3 รายการ" + badge 3 · คลิกขวา → "ย้อน 3 จาก 3" → ยกติ๊กเก่าสุด → "ย้อน 2 จาก 3" → ยืนยัน → DB Elevator+Lobby กลับ active, Dining ยัง off · SkipForward คลิกขวา → "ทำซ้ำ 2 จาก 2" → ทำซ้ำ → ทั้งคู่ off — 🐛 hint redo โชว์คำว่า "ย้อน" → แยก `sch.stepPickRedoHint` — คืน dev data แล้ว — ดู `CHANGELOG.md` [0.4.30] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: restore point ลง DB ซิงก์ข้ามเครื่อง · ลาก/วางไฟล์ JSON ทั้งหน้า · ย้อน/ทำซ้ำขั้นตอนใหญ่ (group undo) · sandbox visual diff ✅**)
- **Restore Points ลง DB** — ตารางใหม่ `scheduler_snapshots` (jsonb) + migration 0013 + API `/api/scheduler-snapshots` GET/POST/DELETE (auth+permission+audit) — `schedulerSnapshotApi` ใน client — บันทึก/ลบ/auto-snapshot sandbox เขียน DB → **เปิดจากเครื่องอื่นเห็นสแนปชอตเดียวกัน** (localStorage เดิมเป็น fallback offline) · กู้คืนใช้ `applySchedulerBackup` เดิม
- **ลาก/วางไฟล์ JSON ทั้งหน้า** — drop zone บน root (เช็ค Files ใน dataTransfer.types) → overlay "ปล่อยเพื่อนำเข้า" + drop = เปิด diff preview ทันที ไม่ต้องคลิกเลือกไฟล์
- **ย้อน/ทำซ้ำขั้นตอนใหญ่ (group undo)** — `HistoryEntry.grp`: แก้ไขต่อเนื่อง <60 วิ อยู่กลุ่มเดียว → ปุ่ม SkipBack/SkipForward ย้อน/ทำซ้ำทั้งกลุ่มทีเดียว — serialize PATCH (store คืน Promise → `await` ทีละตัว) กัน DB เรียงสลับ · pop stack ก่อน apply กัน double-click
- **Sandbox visual diff** — แถบโหมดทดลองแสดงสรุป (เปลี่ยน/เพิ่ม/ลบ/สี) + รายการก่อน→หลัง · ไฮไลต์กฎที่เปลี่ยน: การ์ด List ring เขียว + badge "เปลี่ยน" · ชิป Month ring เขียว — หายเองเมื่อ revert/commit
- 🐛 เจอ/แก้: import ที่**เพิ่มกฎล้วน ๆ** ไม่สร้าง history — `sectionChanged` ไม่เห็น id ใหม่ใน `after` → แก้เพิ่มเงื่อนไข → Ctrl+Z หลัง import เพิ่มล้วน ๆ ลบกฎจริง · PATCH เรียงสลับตอนย้อนหลายขั้น → serialize ใน `upsertSchedules`/`applyUndo`/`applyRedo`
- i18n 3 ภาษา 11 คีย์ — typecheck 0 + build ผ่าน + integration 18/18 — เทสจริง: บันทึก snapshot → reload (จำลองเครื่องอื่น) → เห็นจาก API → ปิด Dining → กู้คืน → DB กลับ active → ลบ (ตารางว่าง) · drop JSON ทั้งหน้า → modal diff → ยืนยัน → DB สร้างกฎ → Ctrl+Z ลบ · ปิดกฎ 3 ข้อ → SkipBack ครั้งเดียวคืนทั้ง 3 → SkipForward ทำซ้ำทั้ง 3 → SkipBack baseline · sandbox: ปิด Dining → "เปลี่ยน 1" + detail "● เปิด → ○ ปิด" + การ์ด/ชิป ring เขียว → revert หาย — คืน dev data แล้ว — ดู `CHANGELOG.md` [0.4.29] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: undo/redo ระดับระบบสำหรับ import/กู้คืน · โหมดทดลอง sandbox (auto-snapshot + undo ไม่จำกัด + commit/revert) · diff preview ค้นหา + ติ๊กเลือกเฉพาะกฎ · ซิงก์ลากสดรวม resize + วางบน legend ✅**)
- **Undo/Redo ระดับระบบสำหรับ import/กู้คืน** — `applySchedulerBackup` บันทึก **1 entry ก้อนเดียว** (ก่อน/หลัง ทั้งหมด: กฎ + สีเพลย์ลิสต์) ก่อนเขียน → **Ctrl+Z หลัง import = ย้อนกลับทุกอย่าง** (ลบกฎที่สร้าง, คืนกฎที่ถูกเขียนทับ) · redo ทำซ้ำทั้งก้อน · อัตโนมัติ ไม่ต้องกดบันทึก
- **โหมดทดลอง (sandbox)** — ปุ่ม "ทดลอง" → **สแนปชอตอัตโนมัติ** (localStorage "Auto-snapshot <เวลา>") + แถบโหมดทดลอง → แก้ไข/ลากได้ตามปกติ แต่ **undo ไม่จำกัด** (ปิดการตัด 50 entry ใน `commitHistory` ระหว่าง sandbox) → จบด้วย **บันทึกผล** (commit: เก็บผล + ลบสแนปชอต) หรือ **คืนค่า** (revert: กู้คืนสแนปชอต + ลบ)
- **Diff preview: ค้นหา + ติ๊กเลือกเฉพาะกฎ** — ช่องค้นหา (ชื่อ/id) + เลือกทั้งหมด/ไม่เลือก + checkbox รายรายการ → สรุป (ใหม่/อัปเดต/ไม่เปลี่ยน/**เลือก X/Y**) คำนวณจากที่ติ๊กสดๆ → ยืนยันนำเข้าเฉพาะรายการที่เลือก
- **ซิงก์ลากสดครบทุกรูปแบบ** — ขยาย `scheduler-drag-sync` payload ด้วย `mode/edge/plDrop`: **resize** → แท็บอื่นเห็นบล็อกขยาย/ย่อ (ความสูงเปลี่ยน, top คงเดิม + ring เฉพาะคอลัมน์เป้า) · **วาง legend** → ไฮไลต์เพลย์ลิสต์เป้า (ring ขาว) · `end` ล้างหมด
- 🐛 เจอ/แก้: remote resize ghost ใช้ `curMin` เป็น top (บล็อกกระโดดตาม cursor) — แก้: ใช้ curMin เฉพาะโหมด move
- i18n 3 ภาษา 15 คีย์ — typecheck 0 + build ผ่าน + integration 18/18 — เทสจริง: sandbox → ปิด Dining → คืนค่า → DB กลับ active + snapshot ลบ → Ctrl+Z หลังคืนค่า → Dining กลับ off → redo → baseline · import 4 กฎ → ค้นหา "sch-9" → 2 แถว → ยกติ๊ก sch-901 → "ใหม่ 1 · เลือก 3/4" → ยืนยัน → DB มี sch-900 ไม่มี sch-901 → Ctrl+Z ลบ sch-900 + คืนเวลา sch-001 · remote resize (curMin 660) → บล็อกสูง 160px top คงเดิม + ring คอลัมน์ จ. · plDrop → legend ไฮไลต์ · end → ล้าง — คืน dev data แล้ว — ดู `CHANGELOG.md` [0.4.28] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: Shift snap ชั่วโมงเต็มใน week strip · ซิงก์ลากสดข้ามแท็บ (live ghost) · import ไฟล์ต่างระบบ + schema mapping + diff preview · restore point สแนปชอต ✅**)
- **Shift snap ชั่วโมงเต็ม** — กด Shift ค้างตอนลากชิป/บล็อกมาวางบน strip → snap เป๊ะชั่วโมง (00) แทน 15 นาที (`snapMinRef` ตามคีย์ — เปลี่ยนกลางลากได้) + hint ใน strip
- **ซิงก์ลากสดข้ามแท็บ** — ทุกการลาก (grid+month) broadcast `scheduler-drag-sync` (start/move/end, throttle ~10/s) → **แท็บอื่นเห็น ghost ตามตำแหน่งจริง**: บล็อกกริดเลื่อนตาม curMin/curDay (ring+shadow) · ชิป Month dim · ปล่อย/ยกเลิก → ล้าง (`remoteDrag` ใน store) — toggle on/off ซิงก์อยู่แล้วผ่าน history-sync
- **Import ไฟล์ต่างระบบ + diff preview** — รองรับ array / `{schedules}` / `{rules}` — **schema mapping อัตโนมัติ** (`title|name`, `weekdays|days_of_week|days`, `start|timeStart`, snake_case ฯลฯ, id ไม่มี → generate) — **modal "ตัวอย่างการนำเข้า" แสดง diff ก่อนเขียน** (ใหม่/อัปเดต/ไม่เปลี่ยน + รายการ + ป้าย foreign) → ยืนยัน = เขียนจริง · ไม่มีการเปลี่ยนแปลง → alert
- **Restore Points (สแนปชอต)** — ปุ่ม "จุดคืนค่า" → modal: **บันทึกสแนปชอต** (กฎ + สี + undo/redo stacks — localStorage อยู่รอด reload) + **กู้คืน/ลบ** — ใช้กลไกเดียวกับ export/import (`applySchedulerBackup`) — กู้ก่อน/หลังทดลองด้วย 1 คลิก
- i18n 3 ภาษา 15 คีย์ — typecheck 0 + build ผ่าน + integration 18/18 — เทสจริง: Shift → marker "10:00" (vs "10:15") → drop → DB 10:00 เป๊ะ → Ctrl+Z คืน · จำลองลากสด → บล็อก Lobby ในคอลัมน์ จ. เลื่อน 09:00 + ring (Month: 30 ชิป dim) → end ล้าง · import foreign (title/weekdays/start) → diff modal "ใหม่ 2" → ยืนยัน → DB สร้าง 2 กฎ (weekdays [1,3,5], [0,6]) → ลบ · snapshot: ปิด Dining → กู้คืน → ทุกกฎ active กลับ — คืน dev data แล้ว — ดู `CHANGELOG.md` [0.4.27] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: week strip กรองตามวัน + ย่อ/ขยาย · ลากบล็อก strip ขึ้นกริดเดือน (2 ทิศทาง) · ซิงก์ view/วันที่/การเลือกข้ามแท็บ · Export/Import กฎ JSON รวมประวัติ ✅**)
- **Week strip: คลิกคอลัมน์วันที่ = กรองเฉพาะกฎวันนั้น** (คอลัมน์อื่น dim 30% + header cyan + chip "วัน ✕" เคลียร์) · **ปุ่มย่อ/ขยาย** 150px↔420px (แกนเวลา/เส้นชั่วโมง/บล็อกปรับตาม) — drop calc ใช้ rect ของ `data-strip-grid` → ความสูงไม่มีผลต่อการคำนวณเวลา
- **ลากบล็อกใน week strip ขึ้นไปวางบนกริดเดือน = เปลี่ยนวัน (ลากได้ 2 ทิศทาง)** — `beginChipDrag` ใช้ร่วม (ชิปเดือน + บล็อก strip) — drop บนเซลล์ → `daysOfWeek` remove ต้น + เพิ่มเป้า (dedupe) · เดือนอื่น → view กระโดด · Alt+ลาก = คัดลอก · คลิก = แก้ไข · Ctrl+คลิก = เลือก — ลากจาก strip ใช้โซน auto-scroll แบบ viewport (กัน auto-scroll ผิดทิศ เพราะ strip อยู่ชิดขอบล่างกริด)
- **ซิงก์ view/วันที่/การเลือกข้ามแท็บ** — `viewMode`/`viewDate` ย้ายไปเก็บใน **store** (`schedulerViewMode`/`schedulerViewDate`/`setSchedulerView`) — แท็บหนึ่งสลับ view หรือ ◀▶/วันนี้ → อีกแท็บตามทันที (`scheduler-view-sync`) · การเลือกหลายอีเวนต์ซิงก์ด้วย (`selection-sync`)
- **Export/Import กฎ Scheduler (JSON + ประวัติ)** — ปุ่มใหม่ใน header — **ส่งออก**: ดาวน์โหลด `scheduler-backup-<date>.json` (schedules + playlists color + **undo/redo stacks**) · **นำเข้า**: upsert กฎ (id ซ้ำ = อัปเดต, ใหม่ = สร้าง, normalize เวลา HH:MM) + กู้สี + คืน stacks → alert สรุป — ใช้ย้าย/สำรองระหว่างระบบ
- i18n 3 ภาษา: `sch.stripFilterHint` / `sch.stripFilterClear` / `sch.stripExpand` / `sch.stripCollapse` / `sch.export` / `sch.import` / `sch.importDone` / `sch.importFail` — typecheck 0 + build ผ่าน + integration 18/18 — เทสจริง: filter "จ 17" + ขยาย 150↔420 · ลากบล็อก Dining ขึ้นวาง 09-01 → DB `[2,3,4,5]` + view "ก.ย. 2026" → Ctrl+Z คืน · จำลองแท็บอื่นส่ง view-sync → เปลี่ยนเป็น Week "19–25 ก.ค. 2026" · selection-sync → แถบเลือกขึ้น · export blob ครบโครงสร้าง · import ไฟล์ → sch-002 [0,6] + สี pl-lunch-menu `#f43f5e` เขียน DB จริง + history จากไฟล์ — regression คลิกชิปเปิด modal — คืน dev data แล้ว — ดู `CHANGELOG.md` [0.4.26] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: History dropdown แสดงก่อน→หลัง + คลิก jump ไปกฎ · undo/redo ซิงก์ข้ามแท็บ (BroadcastChannel) · multi-select อยู่รอดข้าม view + ลากชิป Month ลง Week strip · ปุ่มคัดลอก/ลบกฎใน modal + confirm ✅**)
- **History dropdown แสดงรายละเอียดการเปลี่ยนแปลง** — entry เก็บ `detail` (สร้างตอน push): ชื่อกฎ + ก่อน→หลังของ **เวลา/วัน/เพลย์ลิสต์/เปิด-ปิด/ชื่อ** (+ ชื่อ / − ชื่อ สำหรับสร้าง/ลบ · สีเพลย์ลิสต์ `#abc → #def`) — **คลิก entry = กระโดดไปกฎนั้น** (เปิด modal แก้ไข prefilled) หรือเปิดตัวเลือกสีเพลย์ลิสต์ — dropdown w-80 แบบ 2 บรรทัด
- **Undo/Redo ซิงก์ข้ามแท็บ (BroadcastChannel `signage-history-sync`)** — `setUndoStack`/`setRedoStack`/`clearHistory` broadcast stacks → แท็บอื่นรับแล้ว**ซิงก์ stacks + ดึง schedules/playlists ล่าสุด** (`refreshSchedulesAndPlaylists` ใหม่) — กด Ctrl+Z ในแท็บหนึ่งอีกแท็บอัปเดต (รับเฉพาะ stacks → ไม่วนลูป) · หน่วง+debounce 150ms กัน race กับ PATCH
- **Multi-select อยู่รอดข้าม view** — `selectedScheduleIds` ย้ายไปเก็บใน **store** (เดิมเป็น component state ถูกล้างตอนสลับ view) — เลือกชิปใน Month → สลับ Week/Day → บล็อก ring + ลากตัวใดตัวหนึ่ง = ย้ายทั้งกลุ่ม · Esc ล้าง
- **Week strip ใต้กริดเดือน (ลากข้าม view: Month → Week โดยตรง)** — เส้นเวลา 7 วัน × 06:00–22:00 แสดงบล็อกกฎปัจจุบัน — **ลากชิปมาวาง = เปลี่ยนวัน (คอลัมน์) + เวลา (y)** ของทั้งกลุ่ม (delta เท่ากัน) — ไฮไลต์คอลัมน์ + เส้นเวลาปลายทาง · Alt+ลาก = คัดลอก · auto-scroll ระงับระหว่าง hover strip
- **ปุ่มใน modal แก้ไข: คัดลอกกฎ / ลบกฎ** — คัดลอก = สร้างสำเนาจากค่าปัจจุบันทันที (history "Duplicate" → undo ลบ) · ลบ = `window.confirm` กันลบไม่ตั้งใจ (ทั้ง modal + ถังขยะใน List) — history "Delete" → undo สร้างกลับ
- **Fixed บั๊กแฝง:** undo/redo คืนกฎที่ถูกลบ (กฎที่โหลดจาก DB) 400 — store แมป `startTime` เป็น 'HH:MM:SS' แต่ `CreateScheduleSchema` ต้องการ HH:MM → `upsertSchedules` ส่ง item ดิบ → 400 → กฎไม่ถูกสร้างใน DB — แก้ normalize `slice(0,5)` ตอนสร้างใหม่ (พบตอนเทสลบกฎใน modal แล้ว undo — บั๊กนี้ซ่อนมานานเพราะเทสก่อนหน้าใช้กฎที่สร้างในเซสชัน ซึ่งเวลาอยู่ในรูปแบบ HH:MM อยู่แล้ว)
- i18n 3 ภาษา: `sch.historyJump` / `sch.duplicateRule` / `sch.duplicateHint` / `sch.deleteRule` / `sch.confirmDelete` / `sch.weekStripTitle` / `sch.monthToWeekHint` — typecheck 0 + build ผ่าน + integration 18/18 — ดู `CHANGELOG.md` [0.4.25] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: Undo/Redo ครอบคลุมฟอร์ม+สีเพลย์ลิสต์ เก็บใน store + เลือกหลายบล็อก Week/Day ลากพร้อมกัน + Alt+ลากคัดลอก + ปุ่มประวัติ ✅**)
- **Undo/Redo ครอบคลุมการแก้ไขทุกแบบ** (เดิมมีแค่การลาก) — **สร้าง/แก้ไข/ลบกฎผ่านฟอร์ม + สลับ on/off + เปลี่ยนสีเพลย์ลิสต์** ย้อนกลับได้ (Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y — เขียนกลับ DB จริง: กฎที่ลบ undo แล้วสร้างกลับ, กฎที่สร้าง undo แล้วลบ ฯลฯ)
- **ประวัติเก็บใน store (ไม่ใช่ component state) → อยู่รอดข้ามแท็บ** — `undoStack`/`redoStack`/`clearHistory` ใน `useSignageStore` (50 รายการ, label+time+before/after) — **ปุ่ม History ใหม่** → dropdown แสดงรายการล่าสุด 10 + ปุ่ม **ล้างประวัติ**
- **เลือกหลายอีเวนต์ใน Week/Day + ลากพร้อมกัน** (เหมือน Month) — Ctrl/Shift/⌘+คลิกบล็อก = เลือก (ring + แถบ + Esc) — ลากบล็อกที่ถูกเลือก → **ย้ายทุกกฎด้วย delta เดียวกัน** (เวลา/ข้ามวัน) · drop legend = เปลี่ยนเพลย์ลิสต์ทั้งกลุ่ม · เคลียร์การเลือกหลังปล่อย
- **Alt+ลาก = คัดลอกกฎใหม่ (ต้นฉบับคงเดิม)** — Week/Day: สร้างกฎใหม่ที่วัน/เวลาเป้า (history "Duplicate" → undo ลบ) · Month: Alt+ลากชิปข้ามวัน → สร้างกฎใหม่ days dedupe — รองรับลากกลุ่ม
- **Fixed 2 บั๊ก:** (1) undo สีเพลย์ลิสต์ไม่คืนค่า — `mapPlaylist` แมป `color: p.color || undefined` → JSON.stringify ทิ้ง key → แก้โดยส่ง `{ color: item.color ?? '' }` ใน undo/redo (ไม่ส่งทั้งออบเจกต์ → ไม่ลบ/สร้าง items ซ้ำ) (2) Alt+ลากชิป Month 400 — store แมป `startTime` เป็น 'HH:MM:SS' แต่ `CreateScheduleSchema` ต้องการ HH:MM (strict) → normalize `slice(0,5)` ตอนคัดลอก
- i18n 3 ภาษา: `sch.copyHint` / `sch.historyTitle` / `sch.historyEmpty` / `sch.historyClear` / `sch.histMove` / `sch.histResize` / `sch.histDay` / `sch.histPlaylist` / `sch.histCreate` / `sch.histEdit` / `sch.histDelete` / `sch.histDuplicate` / `sch.histColor` — typecheck 0 + build ผ่าน + integration 18/18 — ดู `CHANGELOG.md` [0.4.24] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: ลากชิปข้ามเดือนไกล (grid ขยายอัตโนมัติ) + เลือกหลายอีเวนต์ลากพร้อมกัน + Undo/Redo ทุกการลาก ✅**)
- **ลากชิป Month ข้ามเดือนไกลได้จริง (ไม่ต้องกด ◀▶)** — ชี้ค้างที่ขอบบน/ล่างของกริด → กริด**ขยายแถวเดือนก่อน/ถัดไปอัตโนมัติ** (สูงสุด 14 แถว/ทิศ ≈ 3.5 เดือน) — `monthCells` dynamic (6+top+bottom สัปดาห์) — **2 เฟส**: เฟส 1 (ขอบกริดนอกจอ) เลื่อนหน้าตามปกติ · เฟส 2 (ชี้ใกล้ขอบกริดที่มองเห็น, clamp viewport) **ขยาย 1 แถว/300ms + เลื่อนหน้าตาม 1 แถว** → แถวใหม่โผล่ที่ขอบพอดี (flow ควบคุมได้) — highlight/วันเป้าไล่ตามทุกเฟรม — ปล่อยบนเดือนไกล → `viewDate` กระโดดไปเดือนนั้นก่อนวาง (ใช้ระบบข้ามเดือนเดิม)
- **เลือกหลายอีเวนต์ใน Month + ลากพร้อมกัน** — Ctrl/Shift/⌘+คลิกชิป = เลือก/ยกเลิก (ring ขาว + แถบ "เลือก N รายการ" + Esc ล้าง) — ลากชิปที่ถูกเลือก → **ย้ายทุกกฎพร้อมกัน** (เอาวันต้นออก+เพิ่มวันเป้า, dedupe ต่อกฎ) — คลิกธรรมดายังเปิด modal แก้ไข
- **Undo/Redo ทุกการลากอีเวนต์ (Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y + ปุ่มใน header)** — บันทึก before/after ของกฎที่ถูกลาก → **ย้าย/ขยาย/เปลี่ยนวัน (ชิปเดือน)/เปลี่ยนเพลย์ลิสต์ (drop legend)/ย้ายกลุ่ม** — undo/redo เขียนกลับผ่าน `updateSchedule` (บันทึก DB จริง) — stack 50 · no-op drop ไม่สร้าง history — `snapshotOf` ใช้ `useSignageStore.getState()` เพื่อให้ `after` เห็นค่าที่ update แล้ว
- i18n 3 ภาษา: `sch.selectHint` / `sch.monthExpandHint` / `sch.selectedCount` / `sch.undo` / `sch.redo` — typecheck 0 + build ผ่าน + integration 18/18 — ดู `CHANGELOG.md` [0.4.23] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler: ซ่อน/แสดงเพลย์ลิสต์ + เลือกสีเองบันทึกลง DB + ลากชิปข้ามวัน/ข้ามเดือน + Auto-scroll ครบทุก view ✅**)
- **คลิกชื่อเพลย์ลิสต์ใน legend = ซ่อน/แสดงอีเวนต์** (ทุกมุมมอง Day/Week/Month/List — `visibleSchedules` = schedules − hiddenPlaylists) — ชื่อที่ซ่อนขีดฆ่า+จาง+EyeOff · legend แยกเป็น block ใช้ร่วมทุก view (รวม List) · จำนวนกฎ/หัวข้อวันนับจาก visible
- **เลือกสีเพลย์ลิสต์เอง + บันทึกลง DB** — คลิกวงกลมสี → modal (12 preset + custom hex + color input) → `updatePlaylist({color})` → **DB column `color` ใหม่ (migration 0012)** + validate/types/mapPlaylist/seed — สี custom render inline (hex→rgba 55%) — เทสจริง: custom #16a34a → DB เขียนจริง + บล็อก/swatch เปลี่ยนทันที + reload แล้วยังอยู่
- **ลากชิปข้ามวันใน Month = เปลี่ยนวันของกฎ** — pointer events + capture + highlight เซลล์เป้าหมาย + chip จาง — drop → `daysOfWeek` เอาวันต้นออก+เพิ่มวันเป้า (dedupe — เจอ/แก้บั๊ก duplicate ตอนเทส) → PATCH บันทึกจริง — คลิกชิป (ไม่ลาก) ยังเปิด modal แก้ไข
- **Auto-scroll ขณะลากชิป Month** — rAF loop เลื่อนหน้า (แนวตั้ง) + กริด (แนวนอน) ต่อเนื่องเมื่อชี้ใกล้ขอบ 72px (เร็ว 6–28px/เฟรม ตามความลึก) — เซลล์ที่เลื่อนเข้ามาใต้ cursor อัปเดต highlight/วันเป้าเองทุกเฟรม — หยุดเมื่อออกจากขอบ/ปล่อย/cancel — เทสจริง: ขอบล่าง 400→636 + highlight ตาม (08-31), ขอบบน 636→0, ปล่อยแล้ว loop หยุด + ข้อมูลไม่เปลี่ยน
- **Auto-scroll แบบเดียวกันกับ week/day grid** — ลากอีเวนต์ในกริดเวลา + ชี้ใกล้ขอบบน/ล่าง viewport → เลื่อนหน้าเองต่อเนื่อง (rAF, โซน 72px, เร็ว 6–28px/เฟรม) + แนวนอนผ่านการ์ด (`weekCardRef`) — ตำแหน่งลาก (curMin/curDay) + drop target legend คำนวณใหม่ทุกเฟรมผ่าน tick (`scrollTickUpdaterRef` ร่วมกับ month view) — เทสจริง: จับบล็อก → ขอบล่าง 0→808 (ghost ตาม) → กลางจอหยุด → ขอบบน 808→0 → ปล่อยที่เดิม ข้อมูลไม่เปลี่ยน
- **ลากชิป Month ข้ามเดือนได้จริง** — ชิปจำวันที่ต้น/ปลายทาง (`origYmd`/`curYmd`) — ปล่อยบนวันที่นอกเดือน → **`viewDate` ขยับไปเดือนนั้นก่อนวาง** (เห็นผลทันทีในกริดใหม่) + เปลี่ยนวันตามวันที่วาง (dedupe) — เทสจริง: 8 ก.ย. → 6 ต.ค. → header "ต.ค. 2026" + highlight เซลล์ระหว่างลาก + DB `[1,2,3,4,5,6]` · 16 ส.ค. → 1 ก.ย. → view "ก.ย. 2026" — regression คลิกชิปยังเปิด modal — **คืน dev data แล้ว** (sch-003 = ทุกวัน `[0..6]`, pl-executive-briefing color='')
- **⚠️ ต้อง restart dev server หลัง migration** — tsx จำ schema เก่า → drizzle drop คีย์ `color` เงียบๆ (PUT 200 แต่ไม่เขียน) — restart แล้วเขียนจริง — typecheck 0 + build ผ่าน + integration 18/18 — i18n 6 คีย์ใหม่ 3 ภาษา — ดู `CHANGELOG.md` [0.4.22] — ⏳ ยังไม่ commit / sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler แยกสีเพลย์ลิสต์ + ลากวางเปลี่ยนเพลย์ลิสต์ + เส้นเวลาสด ✅**)
- **สีหลักบล็อก/ชิปอีเวนต์ = สีประจำเพลย์ลิสต์** (12 สี เรียงตาม id — คงที่ ไม่ชนกัน; เปลี่ยนจาก hash เพราะชน 3/6) — day/week: บล็อกสีเพลย์ลิสต์ + แถบซ้าย 4px สี priority · month: ชิปสีเพลย์ลิสต์ + จุด priority · list: จุดสีข้างชื่อ + **Legend** "สีเพลย์ลิสต์" (swatch + ชื่อ) + 6 ระดับ priority + conflict
- **ลากอีเวนต์ไปวางบน legend เพลย์ลิสต์ = เปลี่ยนเพลย์ลิสต์** (`data-pl-drop` + elementFromPoint — ไฮไลต์ตอน hover, คงเวลา/วันเดิม) — เทส simulate pointer events จริง: Elevator → Cafeteria Lunch Specials → บล็อกเปลี่ยนสีทันที + list view ตรง — **คืนข้อมูล dev แล้ว**
- **เส้นเวลาปัจจุบัน Day/Week เคลื่อนสดทุกนาที** — `nowDate` → state + setTimeout ตรงขอบนาที (60s) — เทสจริง: 11:44→229.333px, 11:45→230px ขยับเอง — i18n `sch.playlistLegend` + `sch.dragToPlaylist` 3 ภาษา — typecheck 0 + build ผ่าน + integration 18/18 — ดู `CHANGELOG.md` [0.4.21] — ⏳ ยังไม่ sync prod

### 2026-08-16 — 🤖 โดย Freebuff (**Scheduler ปฏิทินครบ 3 มุมมอง — Day / Week / Month เหมือน Google Calendar ✅**)
- **SchedulerEngine เพิ่มมุมมองวันเดียว (Day) + รายเดือน (Month)** — view switcher 4 ปุ่ม (วัน/สัปดาห์/เดือน/รายการ) + นำทาง **วันนี้/◀ ▶** + ป้ายช่วงวันที่ — เก็บ state `viewDate` กลาง
- **Day view** ใช้กริดเวลาเดียวกับ week (ลากย้าย/ขยาย/สร้างได้ครบ) · **Month view** 6 สัปดาห์ × 7 วัน ชิปสีตาม 6-Level Priority (+N รายการ) — **คลิกวันที่ → Day view** · **คลิกชิป → แก้ไขกฎ**
- **กรองตามวันที่จริง** (`isActiveOnDate` = dayOfWeek + startDate–endDate) ในทุกมุมมอง — เดิม week view กรองแค่วันในสัปดาห์
- **เพิ่มไฮไลต์สัปดาห์ปัจจุบันใน Month view** (ring สีฟ้า + พื้นหลังจาง 7 เซลล์ อา–ส) + ยืนยันปุ่ม **วันนี้** ใน week view กลับมาที่สัปดาห์ปัจจุบันได้ (30 ส.ค.–5 ก.ย. → วันนี้ → 16–22 ส.ค.)
- ยืนยันใน preview ครบ: สลับ view + นำทาง + คลิกวันที่/ชิป + ภาษา EN/ไทย — typecheck 0 + build ผ่าน — ดู `CHANGELOG.md` [0.4.20] — ⏳ ยังไม่ sync prod
...
- เอกสาร: `docs/deploy-security-guide.md` section Webhook Token — ตัวอย่าง curl ครบ + วิธีหมุนเวียน — ดู `CHANGELOG.md` [0.4.17]
...
- [ ] **sync + redeploy prod 0.4.19/0.4.20 (Scheduler ปฏิทินลากได้ + Day/Month view)** — ยังไม่ได้ sync (ต้องผ่าน Z:\ + `redeploy.bat`) — ไม่มี migration ใหม่ ปลอดภัย
