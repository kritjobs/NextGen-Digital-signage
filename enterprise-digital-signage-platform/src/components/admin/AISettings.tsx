  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 p-4 rounded-2xl border border-slate-800">
        <div>
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <span className="text-[10px] bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded-full font-bold">{t('ai.multiProvider')}</span>
          </h2>
        ...
        <div className="flex bg-slate-950 border border-slate-700 rounded-xl p-0.5">
          <button onClick={() => setActiveSection('providers')} className={`px-4 py-1.5 rounded-lg text-xs font-semibold ${activeSection === 'providers' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}>
            <Globe className="h-3 w-3 inline mr-1" /> {t('ai.tabProviders')}
          </button>
          <button onClick={() => setActiveSection('tasks')} className={`px-4 py-1.5 rounded-lg text-xs font-semibold ${activeSection === 'tasks' ? 'bg-slate-700 text-white' : 'text-slate-400'}`}>
            <Zap className="h-3 w-3 inline mr-1" /> {t('ai.tabTasks')}
          </button>
        </div>
      </div>
      ...
      {activeSection === 'providers' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-white">{t('ai.providers', { count: providers.length })}</span>
            <button onClick={() => setIsAddingProvider(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold rounded-lg">
              <Plus className="h-3.5 w-3.5" /> {t('ai.addProvider')}
            </button>
          </div>
          ...
          {providers.map(p => (
            <div key={p.id} className={`bg-slate-900 border rounded-xl p-4 space-y-3 ${p.isEnabled ? 'border-slate-700' : 'border-slate-800 opacity-60'}`}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => handleTestProvider(p.id)} disabled={testingId === p.id}
                    ...
                    {testingId === p.id ? <RefreshCw className="h-3 w-3 animate-spin inline" /> : <Zap className="h-3 w-3 inline" />} {t('ai.test')}
                  </button>
                  {/* Toggle */}
              ...
              <div className="flex items-center gap-3 text-[11px]">
                ...
                <span className="text-slate-600">{t('ai.apiKeyLabel')}{p.apiKeySet ? t('ai.set') : t('ai.notSet')}</span>
                <span className="text-slate-600">{t('ai.modelsLabel')}{p.models?.length || 0}</span>
              </div>
              ...
              {editingProviderId === p.id ? (
                <div className="p-3 bg-slate-950 rounded-xl border border-purple-500/30 space-y-2">
                  <label className="text-[10px] text-slate-400 block">{t('ai.apiKey')}</label>
                  ...
                  <div className="flex justify-end gap-2">
                    <button onClick={() => { setEditingProviderId(null); setEditApiKey(''); }}
                      className="px-3 py-1 text-xs text-slate-400 hover:text-white">{t('ai.cancel')}</button>
                    <button onClick={() => handleSaveApiKey(p.id)} disabled={!editApiKey.trim()}
                      ...
                      <Save className="h-3 w-3 inline mr-1" /> {t('ai.saveKey')}
                    </button>
                  </div>
                </div>
              ) : (
          ...
          {providers.length === 0 && (
            <div className="py-12 text-center bg-slate-900 rounded-xl border-2 border-dashed border-slate-700">
              ...
              <p className="text-sm text-slate-500">{t('ai.noProviders')}</p>
              <button onClick={() => setIsAddingProvider(true)} className="mt-3 text-xs text-purple-400 hover:text-purple-300">{t('ai.addFirstProvider')}</button>
            </div>
          )}
        </div>
      )}
      ...
      {activeSection === 'tasks' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-bold text-white">{t('ai.taskMappingTitle')}</span>
            <span className="text-[10px] text-slate-500">{t('ai.taskMapping')}</span>
          </div>

          {tasks.length === 0 ? (
            <div className="py-12 text-center bg-slate-900 rounded-xl border-2 border-dashed border-slate-700">
              <Sparkles className="h-10 w-10 text-slate-600 mx-auto mb-3" />
              <p className="text-sm text-slate-500">{t('ai.noTasks')}</p>
            </div>
          ) : (
              ...
              {tasks.map(task => (
                <div key={task.id} className={`bg-slate-900 border rounded-xl p-4 space-y-3 ${task.isEnabled ? 'border-slate-700' : 'border-slate-800 opacity-60'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] text-slate-500 block mb-0.5">{t('ai.provider')}</label>
                    ...
                    <div>
                      <label className="text-[10px] text-slate-500 block mb-0.5">{t('ai.model')}</label>
                  ...
                  <div>
                    <label className="text-[10px] text-slate-500 block mb-0.5">{t('ai.systemPrompt')}</label>
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-slate-500">
                    <span>{t('ai.temp')}{task.temperature}</span>
                    <span>{t('ai.maxTokens')}{task.maxTokens}</span>
                    <span>{t('ai.providerCol')}{task.provider?.name || '—'}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-white">{t('ai.addProvider')}</h3>
              <button onClick={() => setIsAddingProvider(false)} className="text-slate-400 hover:text-white"><X className="h-5 w-5" /></button>
            </div>

            <div className="space-y-3 text-xs">
              <div>
                <label className="text-slate-400 block mb-1">{t('ai.providerType')}</label>
              ...
              <div>
                <label className="text-slate-400 block mb-1">{t('ai.displayName')}</label>
              ...
              <div>
                <label className="text-slate-400 block mb-1">{t('ai.baseUrl')}</label>
              ...
              <div>
                <label className="text-slate-400 block mb-1">{t('ai.apiKey')} {newProvider.type === 'ollama' && <span className="text-slate-600">{t('ai.notRequired')}</span>}</label>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-800">
                <button onClick={() => setIsAddingProvider(false)} className="px-4 py-2 text-slate-400">{t('ai.cancel')}</button>
                <button onClick={handleAddProvider} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg">
                  <Plus className="h-3 w-3 inline mr-1" /> {t('ai.addProvider')}
                </button>
              </div>
            </div>
          </div>
